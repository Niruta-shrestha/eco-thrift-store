        ...
        <table className="w-full text-left">
            ...
            <tbody>
                {cart.items.map((item) => (
                    <tr key={item.id}>
                        <td>
                            <Image src={item.image ? item.image : "/thrift-logo.svg"} alt={item.name} width={50} height={50} style={{ height: "auto" }} className="w-[200px] max-w-[200px] max-h-[200px] h-full object-contain" />
                            <span>
                                {item.name}
                            </span>
                        </td>
