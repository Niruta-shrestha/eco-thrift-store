...
export class GetProductsByCategoryController implements Controller {
    ...
    async invoke(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            const isCatagory = await this.getCategoryById.invoke(req.params.categoryId as string);
            if(!isCatagory) {
                res.status(httpStatus.BAD_REQUEST).send("Category does not exist")
                return
            }
            const products = await this.getProductsByCategoryServices.invoke(req.params.categoryId as string);
